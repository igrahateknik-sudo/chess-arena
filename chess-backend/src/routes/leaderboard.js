 ...
 *
 * Cached in Redis for 60 seconds. Cache is busted by LeaderboardCache.invalidateAll()
 * which is called from gameRoom.js endGame() whenever ELO changes.
 */
router.get('/', async (req, res) => {
  try {
    ...
    // Cache hit
    const cached = await LeaderboardCache.get(tc, limit);
    if (cached) {
    ...
    const leaderboard = data
      ...
      .map((u, i) => ({
        ...
        winRate:      u.games_played > 0
          ? Math.round((u.wins / u.games_played) * 100)
          : 0,
      }));

    const result = { leaderboard, timeControl: tc };

    // Store in cache (fire-and-forget)
    LeaderboardCache.set(tc, limit, result).catch(() => {});
